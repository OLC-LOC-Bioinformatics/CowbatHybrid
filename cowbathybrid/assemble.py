...
def trim_illumina(forward_reads, reverse_reads, output_directory, threads, logfile=None):
    ...
    cmd = 'bbduk.sh -Xmx120g -Xms60g in={forward_reads} in2={reverse_reads} out={forward_trimmed} out2={reverse_trimmed} ' \
...
def correct_illumina(forward_reads, reverse_reads, output_directory, threads, logfile=None):
    ...
    cmd = 'tadpole.sh -Xmx180g in={forward_reads} in2={reverse_reads} out={forward_corrected} out2={reverse_corrected} ' \
...
# Madhu introduced the flye assembler to produce long contigs for producing hybrid assembly
# after looking at CowbatHybrid_code2026, the flye.py command has --nano-hq being used, and this was confirmed looking at the logs from assemblies - AC 2026-02-02
def run_flye(long_reads, output_directory, threads, logfile=None):
    cmd = 'flye --nano-hq {long_reads} -t {threads} --out-dir {output_directory}'.format(
...
def run_unicycler(forward_reads, reverse_reads, long_reads, flye_contigs, output_directory, threads, logfile=None, conservative=False):
    ...
    cmd = 'unicycler -1 {forward_reads} -2 {reverse_reads} -l {long_reads} -o {output_directory} -t {threads} ' \
          '--no_correct --min_fasta_length 2000 --existing_long_read_assembly {flye_contigs} --keep 0 --mode {runmode}'.format(
...
def run_porechop(minion_reads, output_directory, threads, logfile=None):
    ...
    cmd = 'porechop -i {minion_reads} -o {chopped_reads} -t {threads}'.format(
        minion_reads=minion_reads,
...
def subsample_via_filtlong(minion_reads, illumina_forward, illumina_reverse, output_directory, target_bases=250000000, logfile=None):
    ...
    cmd = 'filtlong -t {target_bases} -1 {illumina_forward} -2 {illumina_reverse} {minion_reads} > {filtered_reads}'.format(
        target_bases=target_bases,
        illumina_forward=illumina_forward,
        illumina_reverse=illumina_reverse,
        minion_reads=minion_reads,
...
def subsample_minion_reads(minion_reads, output_directory, target_bases=250000000):
    ...
    # reads we want
    filtered_reads = os.path.join(output_directory, 'length_filtered_reads.fastq')
    with open(filtered_reads, 'w') as f:
        if minion_reads.endswith('.gz'):
            for name, seq, qual in readfq(gzip.open(minion_reads, 'rt')):
...
def run_hybrid_assembly(long_reads, flye_contigs, forward_short_reads, reverse_short_reads, assembly_file, gfa_file, output_directory, filter_reads=None, conservative=False, threads=1):
    """
    Trims and corrects Illumina reads using BBDuk, removes adapters from MinION reads, and then runs unicycler. 
    :param long_reads:  Path to minION reads - uncorrected.
    :param flye_contigs: Path to flye assembly to use as starting point for unicycler.
    ...
    :param gfa_file: The name/path of the GFA file you want created
    ...
    :param conservative: Run unicycler in conservative mode. 
    ...
    # Handle read filtering/subsampling if requested
    filtered_reads = None
    if filter_reads: 
        ...
        filtered_reads = subsample_via_filtlong(minion_reads=long_reads,
                                                ...
                                                logfile=logfile)
        long_reads_to_use = filtered_reads
    else: 
        long_reads_to_use = long_reads
    
    # Chop adapters from MinION reads
    logging.info('Chopping adapters from minION reads...')
    chopped_reads = run_porechop(minion_reads=long_reads_to_use,
                                 output_directory=output_directory,
                                 threads=threads,
                                 logfile=logfile)
    ...
    # Copy assembly and GFA files to final locations
